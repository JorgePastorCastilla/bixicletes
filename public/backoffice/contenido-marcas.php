...
<script>
	$(document).ready(function () {
		...
		$("#tablita").on("click",".boton-insertar",function(){
			$("#tablita>.row:nth-of-type(2)").after('<hr>\
				<div class="row"> \
				<form action="CRUD-marcas.php" method="post" class="form-inline col-sm-12"> \
				<div class="col-sm-3 form-group"> \
				<input type="text" class="form-control col-sm-12" name="idInsert" id="id" placeholder="id de marca" disabled> \
				</div> \
				<div class="col-sm-3 form-group"> \
				<input type="text" class="form-control col-sm-12" name="nombreInsert" id="nombre" placeholder="Nombre de marca"  > \
				</div> \
				<div class="col-sm-3 form-group"> \
				<input type="text" class="form-control col-sm-12" name="urlInsert" id="url" placeholder="url"  > \
				</div> \
				<div class="col-sm-3 form-group"> \
	...
	<div id="tablita"  class="col-sm-12">
		...
		<hr>
		<?php $formes = Marca::selectAll();
		foreach ($formes as $value) {
			foreach ($value as $test) { ?>
			<div class="row">
				<form action="CRUD-marcas.php" method="post" class="form-inline col-sm-12">
					<div class="col-sm-3 form-group">
						<input type="text" class="form-control col-sm-12" name="id" id="id" placeholder="id de marca" value='<?php echo $test->getValueEncoded("idMarca"); ?>' disabled>
					</div>
					<div class="col-sm-3 form-group">
						<input type="text" class="form-control col-sm-12 unlock" name="nombre" id="nombre" placeholder="Nombre de marca" value='<?php echo $test->getValueEncoded("nombreMarca"); ?>' disabled>
					</div>
					<div class="col-sm-3 form-group">
						<input type="text" class="form-control col-sm-12 unlock" name="url" id="url" placeholder="url" value='<?php echo $test->getValueEncoded("webURL"); ?>' disabled>
					</div>
					<div class="col-sm-3 form-group">
